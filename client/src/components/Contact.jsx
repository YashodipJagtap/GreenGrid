...
const Contact = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            let API_BASE = "https://greengrid-backend.onrender.com";

            // If we're on localhost, try local server first
            ...
            try {
                ...
                if (!testResponse.ok) {
                    // If the main backend is not available, try the fallback
                    throw new Error("Server not responding");
                }
            } catch (testError) {
                // If both backends fail, use a client-side solution
                console.log("Server not available, using client-side submission");

                // Simulate server delay
                await new Promise(resolve => setTimeout(resolve, 1500));

                // For demo purposes, we'll simulate a successful submission
                setSuccess("Message sent successfully! (Demo mode - server not connected)");
                setFormData({
                    name: "",
                    email: "",
                    message: ""
            ...
            // If server is available, proceed with actual submission
            ...
            // Don't show error if we're already in demo mode
            if (!success) {
                ...
                {error && (
                    <div className="mb-6 p-4 bg-red-50 border border-red-200 rounded-lg">
                        ...
                        {error.includes("server") && (
                            <p className="text-sm text-red-600 mt-2">Using demo mode to show form functionality</p>
                        )}
                    </div>
                )}
